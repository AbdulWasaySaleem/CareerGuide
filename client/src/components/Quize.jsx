...
import { Star, StarHalf, ChevronRight, BarChart3, RefreshCw } from 'lucide-react';
...
const Quiz = () => {
  ...
  const handleOptionSelect = (option) => {
    ...
    if (newAnsweredCount === QUESTIONS_BEFORE_INTERIM_RESULTS && !interimResultShown) {
  ...
  const renderStarRating = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<Star key={`full-${i}`} className="inline-block text-yellow-400" size={18} fill="currentColor" />);
    }
    
    if (hasHalfStar) {
      stars.push(<StarHalf key="half" className="inline-block text-yellow-400" size={18} fill="currentColor" />);
    }
    
    const emptyStars = 5 - stars.length;
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<Star key={`empty-${i}`} className="inline-block text-gray-300" size={18} />);
    }
  ...
  const getAccuracyLevel = () => {
    if (answeredCount >= 20) return { label: 'Highly Accurate', color: 'text-green-600' };
    if (answeredCount >= 15) return { label: 'Very Accurate', color: 'text-blue-600' };
    if (answeredCount >= 10) return { label: 'Moderately Accurate', color: 'text-yellow-600' };
    ...
    return (
      <div className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-lg">
        <div className="text-center py-8">
          ...
          <p className="text-xl font-bold mb-2 text-red-600">Oops! Something went wrong</p>
      ...
      <div className={`max-w-4xl mx-auto mt-10 transition-opacity duration-500 ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6">
            <h2 className="text-2xl font-bold text-center">Your Career Path Results</h2>
            <p className="text-center text-blue-100 mt-1">Based on {answeredCount} questions</p>
          </div>
          ...
          <div className="p-6">
            <div className="bg-blue-50 p-5 rounded-xl border border-blue-100 mb-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-sm font-medium ${accuracyLevel.color} bg-opacity-20 px-3 py-1 rounded-full`}>
                  {accuracyLevel.label}
                </span>
              </div>
              <h3 className="text-3xl font-bold text-gray-800">{recommendedField}</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              ...
              <button
                onClick={restartQuiz}
                className={`w-full py-3 ${hasMore ? 'bg-gray-100 text-gray-800 hover:bg-gray-200' : 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white hover:from-indigo-700 hover:to-blue-700'} rounded-lg transition shadow-md flex items-center justify-center`}
              >
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center text-sm text-gray-500 mb-2">
              ...
              <div>
                ...
                {hasMore && <span className="text-blue-600 ml-1">• {totalQuestions - questions.length} more available</span>}
              </div>
            </div>
            ...
            {answeredCount < QUESTIONS_BEFORE_INTERIM_RESULTS && (
              <div className="mt-2 text-xs text-gray-500 text-right">
                Results will show after {QUESTIONS_BEFORE_INTERIM_RESULTS - answeredCount} more questions
              </div>
            )}
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-5 rounded-xl mb-6 border border-blue-100">
            ...
            <h2 className="text-xl font-bold text-gray-800">{currentQuestion.questionText}</h2>
          </div>
            ...
            {currentQuestion.options.map((option, index) => (
              <button
                ...
                className={`
                  ...
                  ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-blue-50 hover:border-blue-300 hover:shadow-md'}
                `}
              >
                <div className="flex items-center">
